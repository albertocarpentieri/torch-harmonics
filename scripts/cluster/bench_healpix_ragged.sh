...
# Extra flags, off by default. CUTOFF_DEG overrides the one-grid-spacing default
# radius; SKIP_REFERENCE=1 drops the torch arm, which is what makes a wide radius
# affordable -- neighbours per point grow with the square of the radius, and the
# reference times a dense gather over all of them.
EXTRA_ARGS=""
[[ -n "${CUTOFF_DEG:-}" ]] && EXTRA_ARGS="${EXTRA_ARGS} --cutoff-deg ${CUTOFF_DEG}"
[[ "${SKIP_REFERENCE:-0}" == "1" ]] && EXTRA_ARGS="${EXTRA_ARGS} --skip-reference"
[[ "${SKIP_EQUIANGULAR:-0}" == "1" ]] && EXTRA_ARGS="${EXTRA_ARGS} --skip-equiangular"

[[ -f "${CONTAINER}" ]] || { echo "ERROR: missing ${CONTAINER}" >&2; exit 2; }

mkdir -p "${PROJECT}/logs"

echo "=== job ${SLURM_JOB_ID:-?} | bench | workdir=${WORKDIR} | nsides=${NSIDES} | dtypes=${DTYPES} | extra=${EXTRA_ARGS:-none} ==="
